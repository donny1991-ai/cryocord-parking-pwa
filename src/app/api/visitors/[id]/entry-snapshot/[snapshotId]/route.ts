...
import {
  ...
  replaceVisitorEntrySnapshot,
} from "@/lib/server/entry-snapshots";
import {
...
export async function PUT(
  ...
  try {
    ...
    const result = await replaceVisitorEntrySnapshot({
      visitorId,
      snapshotId,
      actor,
    ...
    return entrySnapshotErrorResponse(error, { requestId, visitorId, snapshotId, actorId });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const result = await removeVisitorEntrySnapshot({
      visitorId,
      snapshotId,
      actor,
    });
...
function entrySnapshotErrorResponse(
  error: unknown,
  context: {
    requestId: string;
    visitorId: string;
  ...
  if (authError) {
    return jsonWithRequestId({ error: authError.error, requestId: context.requestId }, authError.status, context.requestId);
  }

  if (error instanceof EntrySnapshotStorageConfigError) {
    logEntrySnapshotError(error, { ...context, status: 503 });
    return jsonWithRequestId({ error: error.message, requestId: context.requestId }, 503, context.requestId);
  }

  if (error instanceof EntrySnapshotNotEligibleError) {
    return jsonWithRequestId({ error: error.message, requestId: context.requestId }, 409, context.requestId);
  }

  const message = error instanceof Error ? error.message : "Unable to update entry snapshot.";
  if (
    ...
    return jsonWithRequestId(
      { error: "Entry snapshot database migration has not been applied.", requestId: context.requestId },
      503,
      context.requestId,
    );
  }
  if (message.includes("not found")) {
    return jsonWithRequestId({ error: message, requestId: context.requestId }, 404, context.requestId);
  }
  if (
    ...
    message.includes("delete entry snapshot from storage")
  ) {
    return jsonWithRequestId({ error: message, requestId: context.requestId }, 400, context.requestId);
  }

  logEntrySnapshotError(error, { ...context, status: 500 });
  return jsonWithRequestId({ error: "Unable to update entry snapshot.", requestId: context.requestId }, 500, context.requestId);
}
